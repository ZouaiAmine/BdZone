...
public class AdminManagement implements AdminManagementRemote {
	...
	public List<Category> chercherCategParNom(String nom) {
		TypedQuery<Category> query = entityManager.createQuery("Select l from Category l where l.nom=:n",
				Category.class);
	...
	public List<SuperVisor> listerSupervisorsCateg(Category category) {
		...
		TypedQuery<SuperVisor> query = entityManager
				.createQuery("select e from SuperVisor e where e.category=:category", SuperVisor.class);
	...
	public List<Question> listerQuestionByCateg(CategoryType categoryType) {
		TypedQuery<Question> query = entityManager
				.createQuery("select e from Question e where e.categoryType=:categoryType", Question.class);
...
/*
 * @Override public void affecterComptTech(Competence competence, Technicien
 * technicien) { technicien = em.find(Technicien.class, technicien.getCin());
 * em.persist(competence); if (technicien.getCompetences() == null) {
 * List<Competence> competences = new ArrayList<Competence>();
 * competences.add(competence); technicien.setCompetences(competences);
 * 
 * // à chaque fois qu'on ajoute une nouvelle information à une entité //
 * existante il faut faire un merge em.merge(technicien); } else { // Il faut
 * toujours récupérer l'ancienne liste et y rajouter de // nouveaux éléments
 * technicien.getCompetences().add(competence); em.merge(technicien); }
 * 
 * }
 * 
 * @Override public List<Competence> listerCompTechnicien(Technicien technicien)
 * { technicien = em.find(Technicien.class, technicien.getCin()); // il faut
 * ajouter le fetch eager sur la list de competences dans // technicien return
 * technicien.getCompetences(); }
 * 
 * @Override public Employe authentifier(String login, String password) {
 * Employe employe = null; TypedQuery<Employe> query = em .createQuery(
 * "select e from Employe e where e.login=:login and e.password=:password ",
 * Employe.class); query.setParameter("login", login);
 * query.setParameter("password", password); try { return
...
/*
 * 
 * @Override public String supprimerLabo(Labo labo) { // il faut supprimer les
 * liens avec employé (FK) pour pouvoir supprimer // le labo TypedQuery<Employe>
 * query = em.createQuery("select e from Employe e where e.labo=:labo",
 * Employe.class); query.setParameter("labo", labo); List<Employe> emps =
 * query.getResultList(); for (Employe employe : emps) { employe.setLabo(null);
 * } em.remove(em.merge(labo)); return "success";
 * 
 * }
 */